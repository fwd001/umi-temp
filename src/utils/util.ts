...
export const parseResponse = (str: any) => {
  ...
  try {
    ...
    const response = JSON.parse(changeLongToString(newStr));
    return response;
  } catch (e) {
    const reg = /:\d{17,}/;
    const tag = pickTagStr(newStr);
    // 直接对整个str遍历，逐个处理超长的数据类型
    while (reg.test(newStr)) {
      const strBack = newStr;
      newStr = newStr.replace(
        reg,
        (item: any) => `:"${tag}_r_${item.substr(1)}"`,
      ); // 需转换 _r_ 占位并添加""
      // 变化后解析失败，判定为无需转换的内容。回退并添加无需转换标识
      if (!canJsonParse(newStr)) {
        newStr = strBack.replace(
          reg,
          (item: any) => `:${tag}_n_${item.substr(1)}`,
        ); // 不需要转换 _n_ 占位
      }
    }
    const result = newStr
...
export const getDescByCode = (targetCode: any, list = []) => {
  let desc = '';
  const matchItem: any = list.find(
    (item: any) => item.code === Number(targetCode),
  );
  if (matchItem) {
    desc = matchItem.desc;
  }
 ...
 * 动态添加js
 * */
export const dynamicLoad = (src: string, call?: (...args: any[]) => any) => {
...
export function findCurNode(
  ...
  tree.forEach((item: any) => {
    ...
    if (item.children && item.children.length) {
      const findChildren = findCurNode(item.children, curKey, keyField, _node);
      if (findChildren) {
        _node = findChildren;
      }
    }
